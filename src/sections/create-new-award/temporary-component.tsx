...
export default function TemporaryComponent() {
  ...
  const { accordions } = useRatingCriteria();
  ...
  const handlePublish = async () => {
    try {
      const response = await axiosInstance.post("/awards/publish", {
        id: award.id,
      });
      setDialogOpen(true);
      setDialogTitle("Successfully");
      ...
      if (error.result.status === 715) {
        setDialogOpen(true);
        setDialogTitle("Error");
        setDialogTitle("");
        setType("error");
        setDialogMessage("MISSING MANDATORY MILESTONES");
        return;
      }
      if (error.result.status === 716) {
        setDialogOpen(true);
        setDialogTitle("Error");
        setDialogTitle("");
        setType("error");
  ...
  useEffect(() => {
    if (milestones.length > 0) {
      ...
      milestones
        ...
        .forEach((m) => {
          const alreadyExists = updatedEntries.some(
            (entry) => entry.type === "002" && entry.milestoneId === m.id
          );
      ...
      if (updatedEntries.length !== timelineEntries.length) {
        setTimelineEntries(updatedEntries);
      }
    }
  }, [milestones, timelineEntries]);
  ...
  const handleAddEntry = (type) => {
    ...
    setTimelineEntries((prev) => {
      console.log("prevvvvvvadasdasdasd", prev);
      console.log("newEntrytweasdasdsa", newEntry);
  ...
  const areMandatoryFieldsFilled = () => {
    ...
    const awardFields = [
      {
        name: "awardName",
        value: language === "En" ? awardData.nameEn : awardData.nameAr,
      },
      { name: "organizingHost", value: awardData.organizingHost },
      { name: "targetedAudience", value: awardData.targetedAudience },
      {
        name: "aboutAward",
        value:
          language === "En" ? awardData.objectiveEn : awardData.objectiveAr,
      },
    ];
    console.log("award field ", awardFields);

    const isAwardDataComplete = awardFields.every((field) => {
      const isValid = field.value?.toString().trim() !== "";
      if (!isValid) {
        console.log(`Award field '${field.name}' is invalid:`, field.value);
      }
      return isValid;
    });
    const isTimelineComplete =
      timelineEntries.length > 0 &&
      timelineEntries.every((entry, index) => {
        if (entry.type === "002") {
          const hasMilestoneId = !!entry.milestoneId;
          const hasDate = entry.date !== null;

          if (!hasMilestoneId || !hasDate) {
            console.log(`Milestone entry ${index} is incomplete:`, {
              milestoneId: entry.milestoneId,
              date: entry.date,
              hasMilestoneId,
              hasDate,
        ...
        // For Events (type "001")
          ...
          if (!hasTitle || !hasDate) {
            console.log(`Event entry ${index} is incomplete:`, {
    ...
    // 3. Judge Terms
    const areJudgeTermsComplete =
      Judgeterms.length > 0 &&
      Judgeterms.every((term, index) => {
        const termText = language === "Ar" ? term.ruleAr : term.ruleEn;
        const isValid = termText?.trim() !== "";

        if (!isValid) {
          console.log(`Judge term ${index} is invalid:`, term);
        }
        return isValid;
      });

    // 4. Applicant Terms
    const areApplicantTermsComplete =
      applicantSettings.applicantRules.length > 0 &&
      applicantSettings.applicantRules.every((term, index) => {
        const termText = language === "En" ? term.ruleEn : term.ruleAr;
        const isValid = termText?.trim() !== "";

        if (!isValid) {
          console.log(
            `Applicant term ${index} is invalid (${language}):`,
            term
          );
        }
        return isValid;
    ...
    const arePrizesComplete =
      prizes.length > 0 &&
      prizes.every((prize, index) => {
        const isValid = typeof prize.value === "number" && prize.value > 0;

        if (!isValid) {
          console.log(`Prize ${index} is invalid:`, prize);
        }
        return isValid;
      });
    ...
    if (!isApplicantTypeSelected) {
      console.log("Applicant type is not selected:", applicantType);
    }
    ...
    console.log("Mandatory fields check result:", {
      isAwardDataComplete,
      isTimelineComplete,
      areJudgeTermsComplete,
      areApplicantTermsComplete,
